...
const getName = (node, src) => {
  let name = (node.name ? node.name.escapedText : undefined)
...
function checkType(node) {
  console.group((node.name ? node.name.escapedText : undefined));
...
const convertParams = (jsDoc = '', node, src) => {
  const parameters = (node.name ? node.name.escapedText : undefined) || node.parameters
